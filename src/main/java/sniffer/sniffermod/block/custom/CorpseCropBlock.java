...
public class CorpseCropBlock extends TallPlantBlock implements Fertilizable {
    public static final MapCodec<CorpseCropBlock> CODEC = createCodec(CorpseCropBlock::new);
    ...
    @Override
    public MapCodec<CorpseCropBlock> getCodec() {
        return CODEC;
    }

    public CorpseCropBlock(AbstractBlock.Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        boolean bl = random.nextInt((int)(25.0F) + 1) == 0;
        if (bl) {
    ...
    @Nullable
    private CorpseCropBlock.LowerHalfContext getLowerHalfContext(WorldView world, BlockPos pos, BlockState state) {
        if (isLowerHalf(state)) {
            return new CorpseCropBlock.LowerHalfContext(pos, state);
        } else {
            BlockPos blockPos = pos.down();
            BlockState blockState = world.getBlockState(blockPos);
            return isLowerHalf(blockState) ? new CorpseCropBlock.LowerHalfContext(blockPos, blockState) : null;
        }
    }

    @Override
    public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state) {
        CorpseCropBlock.LowerHalfContext lowerHalfContext = this.getLowerHalfContext(world, pos, state);
